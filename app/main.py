...
if __name__ == "__main__":
    ...
    if comm.rank == 0:
        ...
        R_hat_hat, H_hat_hat, W_hat_hat, a_hat_hat, L_c_p, U_c_p, u_bar_2d_hat_hat, p_bar_2d_hat_hat, G_hat_hat, Re_p = second_nondimensionalisation(R_hat, H_hat, W_hat,  a, L_c, U_c, G_hat, Re, u_bar_2d_hat, p_bar_2d_hat, U_m_hat, print_values=False)
    ...
    force_grid = F_p_grid(R_hat_hat, H_hat_hat, W_hat_hat, a_hat_hat, G_hat_hat, Re_p, L=4*max(H_hat_hat, W_hat_hat), particle_maxh=0.08*a_hat_hat, global_maxh=0.3*min(H_hat_hat, W_hat_hat), eps=0.2*a_hat_hat)

    grid_values = force_grid.compute_F_p_grid_ensemble(N_r=15, N_z=15, u_bg_data_np=u_data_np, p_bg_data_np=p_data_np)

    if comm.rank == 0:
        r_vals, z_vals, phi, Fr_grid, Fz_grid = grid_values
        force_grid.plot_paper_reproduction(L_c_p, L_c, r_vals, z_vals, phi, Fr_grid, Fz_grid)