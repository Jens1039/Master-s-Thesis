...
class background_flow_G_approximated:
    ...
    def __init__(self, R, H, W, Q, rho, mu, a):
        ...
        self.mu = mu
        self.a = a
        self.Re = (rho*Q*2/(W + H))/mu
        self.mesh2d = RectangleMesh(120, 120, self.W, self.H, quadrilateral=False, diagonal="crossed")
    ...
    def solve_2D_background_flow(self, H_1_seminorm=False):
        ...
        # "CG": "Continuous Galerkin" = "P" different names just for historical reasons (FENICS) (continuity on nodes is internally ensured)
        # "DG": "Discontinuous Galerkin" the basis functions are the same, but when assembling the matrix continuiy on the nodes is not ensured
        # Taylor - Hood: ("CG" k, "CG" k-1) => LBB - stable, approximatively divergence free, not local mass preservance
        # Scott - Vogelius ("CG" k, "DG" k-1) => LBB - stable on special meshs (e.g. barycentrically refined (every edge, surface has a midddle point))
        ...
        if H_1_seminorm:
            print(sqrt(assemble(inner(grad(self.u_sol), grad(self.u_sol))*dx)))
...
class background_flow:
    ...
    def __init__(self, R, H, W, Q, Re):
        ...
        # D_h = 4A/P, and for a rectangular duct: D_h = 2 W H / (W + H)
        self.D_h = 2.0 * W * H / (W + H)
        self.U_c = Q / (W * H)  # mean (bulk) velocity
    ...
    def solve_2D_background_flow(self, H_1_seminorm=False):
        ...
        V = VectorFunctionSpace(self.mesh2d, "CG", 2, dim=3)
        Q = FunctionSpace(self.mesh2d, "CG", 1)
        G_space = FunctionSpace(self.mesh2d, "CG", 1)
        ...
        # Shortcuts for readability (derivatives w.r.t. non-dimensional coordinates)
        def del_r(f):  return Dx(f, 0)
        def del_z(f):  return Dx(f, 1)

        # weak form of the continuity equation (dimensionless)
        F_cont = q * (del_r(u_r) + del_z(u_z) + u_r / (self.R + r)) * (self.R + r) * dx

        # weak form of the radial-component (dimensionless form of eq. (NS_theta_independent_pressure_split_dimless))
        F_r = (
            # convective and centrifugal terms
            (u_r * del_r(u_r) + u_z * del_z(u_r) - (u_theta**2) / (self.R + r)) * v_r
            # pressure gradient term
            + del_r(p) * v_r
            # viscous terms with 1/Re
            + (1.0 / self.Re) * dot(grad(u_r), grad(v_r))
            + (1.0 / self.Re) * ((1.0 / (self.R + r)) * del_r(u_r) - (u_r / (self.R + r)**2)) * v_r
        ) * (self.R + r) * dx

        # weak form of the azimuthal-component (dimensionless)
        F_theta = (
            # convective terms
            (u_r * del_r(u_theta) + u_z * del_z(u_theta) + (u_theta * u_r) / (self.R + r)) * v_theta
            # driving term with unknown \hat{G}
            - (G_hat * self.R / (self.R + r)) * v_theta
            # viscous terms with 1/Re
            + (1.0 / self.Re) * dot(grad(u_theta), grad(v_theta))
            + (1.0 / self.Re) * ((1.0 / (self.R + r)) * del_r(u_theta) - (u_theta / (self.R + r)**2)) * v_theta
        ) * (self.R + r) * dx

        # weak form of the z-component (dimensionless)
        F_z = (
            # convective terms
            ...
            + del_z(p) * v_z
            # viscous terms with 1/Re
            + (1.0 / self.Re) * dot(grad(u_z), grad(v_z))
            + (1.0 / self.Re) * ((1.0 / (self.R + r)) * del_r(u_z)) * v_z
        ) * (self.R + r) * dx

        # Flow-rate constraint (dimensionless):
        # (D_h^2 / (W H)) \int_{hat S} u_theta (R_hat + r) d hat r d hat z = 1
        # Implemented in weak form with test function g.
        kappa = self.D_h**2 / (self.W * self.H)
        F_flow = g * (kappa * u_theta * (self.R + r) - kappa) * dx

        # Enforce G_hat spatial constancy with respect to (r, z), since pressure variations are captured in the other part of the decomp.
        F_constG = inner(grad(G_hat), grad(g)) * dx

        # Total residual
        F = F_cont + F_r + F_theta + F_z + F_flow  + F_constG
        ...
        nullspace = MixedVectorSpaceBasis(
            ...
            [
                ...
                W_mixed.sub(2),  # G_hat: no special nullspace, but must not be None
            ],
        )
        ...
        solver = NonlinearVariationalSolver(
            ...
            solver_parameters={
                # SNES = Scalable Nonlinear Equation Solver  here: Newton with Line Search as step range criteria
                "snes_type": "newtonls",
                # bt: test the descent criteria ||F(w_k + a_k*d_k)|| < (1 - c a_k)||F(w_k)|| with decreasing a_k until it is valid
                # l2: solves min ||F(x_k + alpha*d_k||_(L^2) over alpha
                "snes_linesearch_type": "l2",
                # Tests for ||F(w_k)||/||F(w_0)|| and stops algorithm once ratio has fallen below
                "snes_rtol": 1e-8,
        ...
        solver.solve()
        u_sol, p_sol, G_hat_sol = w.subfunctions
        self.u_bar = u_sol
        self.p_bar = p_sol
        self.G_hat = float(G_hat_sol.dat.data_ro[0])

        if H_1_seminorm:
            print(sqrt(assemble(inner(grad(self.u_bar), grad(self.u_bar)) * dx)))

        return u_sol, p_sol, self.G_hat
