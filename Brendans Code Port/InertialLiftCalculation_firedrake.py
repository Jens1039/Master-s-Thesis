...
RZ, UVW = ComputeBackgroundFlow(W, H, R, Re = 1.0, nW = 32, nH = 32, scaling = "mean")
...
facet_markers, mesh, cell_tags, facet_tags = GenerateCurvedDuctParticleMesh(W,H,R,px,pz,pr,DL)
...
def calculate_drag_torque(up,p=None,string=None):
    ...
    if MPI_size>1:
        Dx,Dy,Dz,Tx,Ty,Tz = MPI.COMM_WORLD.allreduce(np.array([Dx,Dy,Dz,Tx,Ty,Tz]),MPI.SUM)
...
def reciprocal_coefficients(inertia_term,string=None):
    ...
    if MPI_size>1:
        Fx,Fy,Fz,Tx,Ty,Tz = MPI.COMM_WORLD.allreduce(np.array([Fx,Fy,Fz,Tx,Ty,Tz]),MPI.SUM)
    if MPI_rank==0 and (string is not None):
...
centripetal = MPI.COMM_WORLD.allreduce(centripetal,MPI.SUM)
if MPI_rank==0:
