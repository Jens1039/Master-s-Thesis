...
def second_nondimensionalisation(R, H, W, a, L_c, U_c, Re, bg_flow, print_values=False):

    # characteristic length is now the particle diameter a
    L_c_p = a

    # characteristic velocity is the maximal axial velocity
    U_c_p = bg_flow.U_m * (L_c_p/L_c)

    # We translate the variables from the first nondimensionalisation system to the second one
    R_nd = (L_c/L_c_p) * R
    H_nd = (L_c/L_c_p) * H
    W_nd = (L_c/L_c_p) * W
    a_nd = (1/L_c_p) * a    # Note, that a has not to be unscaled from the first nondimensionalisation, since it is not used for the background flow
    u_bar_nd = (U_c/U_c_p) * bg_flow.u_bar_3d
    p_bar_nd = ((U_c/U_c_p)**2) * bg_flow.p_bar_3d
